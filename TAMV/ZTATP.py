...
def probePlate():
    ...
    prt.gCode('M400')                                               # Wait for the firmware to update the machine state
...
def probeTool(tn):
    ...
    prt.gCode('M558 K0 P5 C"'+pin+'" F100')                         # Define nozzle<>bed wire as probe - adjusted probing speed to 100mm/minute
    prt.gCode('G0 X'+str(tp[0])+' Y'+str(tp[1])+' F10000')          # Move nozzle to spot above flat part of plate
    prt.gCode('G31 P750')                                           # Set probe sensitivity to 750 to prevent false triggers
